...
def make_prediction():
    ...
    for id in track_ids:
        ...
        break
    data = pd.DataFrame(np.array([[text,mood,'emoji']]),columns = ['Lyrics','Mood','Emoji'])
    data.set_index(['Lyrics'], inplace=True)
    data.index.name=None
    return render_template('results.html',tables=[data.to_html(classes='data')],titles = ['Lyrics','Mood','Emoji'],country=countryCode)
